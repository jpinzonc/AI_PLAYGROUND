...
def main() -> None:
    # Get LLM model
    llm_model = st.sidebar.selectbox("Select LLM", options=["llama3", "llama3-70b-8192", "llama3-8b-8192", "mixtral-8x7b-32768"])
    ...
    if "rag_assistant" not in st.session_state or st.session_state["rag_assistant"] is None:
        ...
        Model = 'llama3'
        rag_assistant = ChatOpenAI(api_key = API_KEY,model= Model, base_url="http://localhost:11434/v1", streaming=True)
        # get_groq_assistant(llm_model=llm_model, embeddings_model=embeddings_model)
